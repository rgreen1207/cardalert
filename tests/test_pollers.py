import pytest
import pollers
from tests.conftest import set_poller_get, set_poller_head


# --- Amazon: the important one — first-party vs third-party seller filtering ---

async def test_amazon_first_party_in_stock(fake_response):
    html = '<div id="add-to-cart-button">Add to Cart</div> Ships from and sold by Amazon.com'
    set_poller_get(lambda *a, **k: fake_response(text=html))
    result = await pollers.check_amazon("B0D7QJXK9P")
    assert result["in_stock"] is True
    assert result["raw_status"] == "AVAILABLE"


async def test_amazon_third_party_seller_excluded(fake_response):
...
async def test_amazon_sold_out(fake_response):
...
async def test_amazon_first_party_but_sold_out(fake_response):
...
async def test_amazon_extracts_asin_from_various_url_shapes(fake_response):
    set_poller_get(lambda *a, **k: fake_response(text="Ships from and sold by Amazon.com <div id=\"add-to-cart-button\">x</div>"))
    for identifier in [
        "B0D7QJXK9P",
        ...
        result = await pollers.check_amazon(identifier)
        assert result["raw_status"] == "AVAILABLE"


async def test_amazon_invalid_identifier():
    result = await pollers.check_amazon("not-a-valid-identifier")
...
async def test_target_blocked_or_key_invalid_returns_distinct_status(fake_response):
    ...
    set_poller_get(lambda *a, **k: fake_response(status_code=403, text="Forbidden by Target"))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "BLOCKED_OR_KEY_INVALID"
    assert result["in_stock"] is False
    assert "Forbidden by Target" in result["error_detail"]


async def test_target_401_also_returns_blocked_status(fake_response):
    set_poller_get(lambda *a, **k: fake_response(status_code=401))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "BLOCKED_OR_KEY_INVALID"
    assert "error_detail" in result


async def test_target_rate_limited_returns_distinct_status(fake_response):
    set_poller_get(lambda *a, **k: fake_response(status_code=429, text="Too many requests"))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "RATE_LIMITED"
    assert "Too many requests" in result["error_detail"]


async def test_target_403_with_real_captcha_response_is_classified_correctly(fake_response):
    ...
    set_poller_get(lambda *a, **k: fake_response(status_code=403, text=real_body))
    result = await pollers.check_target("1012055696")
    assert result["raw_status"] == "CAPTCHA_REQUIRED"
    assert "captchaRelativeURL" in result["error_detail"]
    assert result["in_stock"] is False


async def test_target_403_with_perimeterx_marker_is_classified_as_antibot(fake_response):
    ...
    set_poller_get(lambda *a, **k: fake_response(status_code=403, text=block_page))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "BLOCKED_BY_ANTIBOT"
    assert "perimeterx" in result["error_detail"].lower()


async def test_target_403_without_antibot_marker_stays_generic(fake_response):
    ...
    set_poller_get(lambda *a, **k: fake_response(status_code=403, text="Forbidden"))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "BLOCKED_OR_KEY_INVALID"


async def test_target_429_with_captcha_marker_is_classified_as_antibot(fake_response):
    ...
    set_poller_get(lambda *a, **k: fake_response(status_code=429, text=block_page))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "BLOCKED_BY_ANTIBOT"


async def test_target_non_json_response_returns_distinct_status():
    ...
    set_poller_get(lambda *a, **k: HtmlResponse())
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "UNEXPECTED_RESPONSE"
    assert "unusual traffic" in result["error_detail"]


async def test_target_missing_product_key_returns_not_found_instead_of_crashing(fake_response):
    ...
    set_poller_get(lambda *a, **k: fake_response(json_data={"data": {}}))
    result = await pollers.check_target("1011209279")
    assert result["in_stock"] is False
    assert result["raw_status"] == "NOT_FOUND"


async def test_target_completely_empty_response_does_not_crash(fake_response):
    set_poller_get(lambda *a, **k: fake_response(json_data={}))
    result = await pollers.check_target("1011209279")
    assert result["raw_status"] == "NOT_FOUND"


async def test_target_uses_custom_api_key_when_configured(fake_response):
    ...
    import config
    await config.set("target_api_key", "my-own-key-123")
    captured_url = {}
    ...
    set_poller_get(fake_get)
    await pollers.check_target("1011209279")
    assert "key=my-own-key-123" in captured_url["url"]


async def test_target_falls_back_to_shared_key_when_none_configured(fake_response):
    import config
    await config.set("target_api_key", "")
    captured_url = {}
...
async def test_discover_target_api_key_tries_candidate_tcins_first(fake_response):
    ...
    set_poller_get(fake_get)
    key = await pollers.discover_target_api_key(candidate_tcins=["1234567890"])
...
async def test_discover_target_api_key_falls_back_when_candidate_has_no_key(fake_response):
    def fake_get(url, headers=None, timeout=None, **kwargs):
...
async def test_discover_target_api_key_falls_back_when_candidate_fetch_fails(fake_response):
    import requests
...
async def test_discover_target_api_key_no_candidates_uses_fallback_only(fake_response):
    set_poller_get(lambda *a, **k: fake_response(text='"apiKey":"12341234123412341234123412341234"'))
    key = await pollers.discover_target_api_key(candidate_tcins=None)
    assert key == "12341234123412341234123412341234"


async def test_discover_target_api_key_finds_key_via_redsky_url_pattern(fake_response):
    html = 'some page content <script>fetch("https://redsky.target.com/foo?key=abcdef0123456789abcdef0123456789&tcin=1")</script>'
    set_poller_get(lambda *a, **k: fake_response(text=html))
    key = await pollers.discover_target_api_key()
    assert key == "abcdef0123456789abcdef0123456789"


async def test_discover_target_api_key_finds_key_via_apikey_json_field(fake_response):
...
async def test_discover_target_api_key_returns_none_when_not_found(fake_response):
    set_poller_get(lambda *a, **k: fake_response(text="<html>nothing here</html>"))
    assert await pollers.discover_target_api_key() is None


async def test_discover_target_api_key_returns_none_on_network_error():
    import requests

    def raise_error(*a, **k):
        raise requests.RequestException("connection failed")

    set_poller_get(raise_error)
    assert await pollers.discover_target_api_key() is None


async def test_check_pokemon_center_queue_only_detects_queue(fake_response):
    set_poller_head(lambda *a, **k: fake_response(url="https://cardalert.queue-it.net/somequeue"))
    result = await pollers.check_pokemon_center_queue_only("https://www.pokemoncenter.com/product/x")
    assert result["queue_live"] is True


async def test_check_pokemon_center_queue_only_no_queue(fake_response):
    set_poller_head(lambda *a, **k: fake_response(url="https://www.pokemoncenter.com/product/x"))
    result = await pollers.check_pokemon_center_queue_only("https://www.pokemoncenter.com/product/x")
    assert result["queue_live"] is False


async def test_check_pokemon_center_queue_only_falls_back_to_get_on_405(fake_response):
    """Some servers reject HEAD on a given URL — must still detect the
    queue via a GET fallback rather than giving up."""
    set_poller_head(lambda *a, **k: fake_response(status_code=405))
    set_poller_get(lambda *a, **k: fake_response(url="https://cardalert.queue-it.net/somequeue"))
    result = await pollers.check_pokemon_center_queue_only("https://www.pokemoncenter.com/product/x")
    assert result["queue_live"] is True


async def test_check_pokemon_center_queue_only_falls_back_to_get_on_network_error(fake_response):
    import requests

    def raise_error(*a, **k):
        raise requests.RequestException("connection reset")

    set_poller_head(raise_error)
    set_poller_get(lambda *a, **k: fake_response(url="https://www.pokemoncenter.com/product/x"))
    result = await pollers.check_pokemon_center_queue_only("https://www.pokemoncenter.com/product/x")
    assert result["queue_live"] is False


async def test_target_sends_browser_like_headers(fake_response):
...
async def test_target_in_stock(fake_response):
    payload = {
        "data": {
            "product": {
    ...
    set_poller_get(lambda *a, **k: fake_response(json_data=payload))
    result = await pollers.check_target("1011209279")
    assert result["in_stock"] is True
    assert result["price"] == 24.99


async def test_target_sold_out(fake_response):
    payload = {
        "data": {
            "product": {
                "price": {"current_retail": 24.99},
...
async def test_bestbuy_no_api_key_configured():
    result = await pollers.check_bestbuy("6418599")
    assert result["raw_status"] == "NO_API_KEY"
    assert result["in_stock"] is False


async def test_bestbuy_with_api_key(fake_response):
    import config
    await config.set("bestbuy_api_key", "fake-key")
...
async def test_lgs_generic_in_stock(fake_response):
    html = '<button>Add to Cart</button> <span class="price">$29.99</span>'
    set_poller_get(lambda *a, **k: fake_response(text=html))
    result = await pollers.check_lgs_generic("https://example-lgs.com/product/x")
    assert result["in_stock"] is True
    assert result["price"] == 29.99


async def test_lgs_generic_sold_out(fake_response):
...
async def test_verify_shopify_store_detects_shopify(fake_response):
    payload = {"products": [{"id": 1}, {"id": 2}]}
    set_poller_get(lambda *a, **k: fake_response(json_data=payload, status_code=200))
    result = await pollers.verify_shopify_store("mylocalshop.com")
    assert result["is_shopify"] is True


async def test_verify_shopify_store_detects_non_shopify(fake_response):
    set_poller_get(lambda *a, **k: fake_response(status_code=404))
    result = await pollers.verify_shopify_store("notashopifystore.com")
    assert result["is_shopify"] is False


async def test_verify_shopify_store_handles_connection_error():
    import requests
    ...
    set_poller_get(raise_error)
    result = await pollers.verify_shopify_store("unreachable.com")
    assert result["is_shopify"] is False
